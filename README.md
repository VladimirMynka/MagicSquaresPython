...
- [Магические квадраты](#магические-квадраты)
  ...
  - [Алгебраические свойства](#алгебраические-свойства)
    ...
    - [Чарующие квадраты](#чарующие-квадраты)
      - [Таблица 1. Таблица умножения в линейном пространстве m₀-m₄](#таблица-1-таблица-умножения-в-линейном-пространстве-m-m)
  ...
  - [Подходы к решению проблемы квадратных квадратов](#подходы-к-решению-проблемы-квадратных-квадратов)
    ...
    - [Анализ с использованием гауссовых целых чисел](#анализ-с-использованием-гауссовых-целых-чисел)
...
<details>
  <summary>Дополнительное преобразование базисов</summary>
  <blockquote>
...
<details>
  <summary>Формулы умножения квадратов</summary>
  <blockquote>
...
<details>
<summary>Минимальный целочисленный магический квадрат</summary>
<blockquote>
...
<details>
<summary>Анализ с использованием гауссовых целых чисел</summary>
<blockquote>

### Анализ с использованием гауссовых целых чисел

**Теорема 1.** Если число `M` имеет факторизацию 
```
m = 2ᵃ * p₁ᵇ¹ * p₂ᵇ² * ... * pₖᵇᵏ * q₁ᶜ¹ * q₂ᶜ² * ... * qₙᶜⁿ
```
Где `pᵢ` - простые числа вида `4k + 1`, а `qᵢ` - простые числа вида `4k + 3`.

Тогда:

- m не может быть представлено в виде суммы двух квадратов целых чисел, если хотя бы одно из чисел `cᵢ` нечётно.
- m представляется в виде суммы двух квадратов целых чисел ровно `(b₁ + 1) * ... * (bₖ + 1) / 2` способами, если хотя бы одно из чисел `bᵢ` нечётно.
- m представляется в виде суммы двух квадратов целых чисел ровно `((b₁ + 1) * ... * (bₖ + 1) + 1) / 2` способом, если все числа `bᵢ` чётны.

Данная безымянная теорема приведена в [[1](#ссылки)].

О числе `2e²`, где `e²` - центральный элемент квадратного квадрата, известно, что оно представимо в виде суммы двух квадратов целых чисел как минимум пятью различными способами:
```
2e² = e² + e²
2e² = a² + j² (сумма E+x и E-x)
2e² = b² + h² (сумма E-x+y и E+x-y)
2e² = c² + g² (сумма E-y и E+y)
2e² = d² + f² (сумма E-x-y и E+x+y)
```

Это означает, что число `2e²` должно содержать в своей факторизации либо одно простое число вида `4k + 1` в степени 8 или больше, либо хотя бы два простых числа вида `4k + 1` в степени 2 или больше. Отсюда напрямую следует, что `e` должно представляться либо в виде `p₁ * p₂ * o`, либо в виде `p₁⁴ * o`, где `o` - собирательный множитель для всех других простых чисел в факторизации `e`.

Однако в случае, если `e` содержит лишь один простой делитель `p₁` вида `4k + 1`, только в одном из пяти представлений `2e²` в виде суммы квадратов слагаемые не будут делиться на `p₁²`, что в итоге противоречит минимальности квадратного квадрата.

<details>
<summary>Доказательство</summary>
<blockquote>

1. Докажем, что если `e` содержит лишь один простой делитель `p₁` вида `4k + 1` в любой степени, то во всех представлениях `2e²` в виде суммы квадратов, кроме одного, слагаемые будут делиться на `p₁²`.

Поскольку `p₁` - единственный простой делитель `e`, то собирательный множитель `o` не будет расписываться в виде суммы квадратов ненулевых целых чисел. Это означает, что при разложении `2e²` на произведение двух сопряжённых гауссовых чисел, `o` всегда будет входить в оба множителя целиком. Далее, полагая, что `p₁ = (m + ni)(m - ni)` запишем все такие способы расписать число `2e^2 = p ^ 2k * o^2` разделяя сопряжённые числа знаком `|`:

```
(1 + i) * (m + ni)²ᵏ * (m - ni)⁰ | (1 - i) * (m + ni)⁰ * (m - ni)²ᵏ
(1 + i) * (m + ni)²ᵏ⁻¹ * (m - ni)¹ | (1 - i) * (m + ni)¹ * (m - ni)²ᵏ⁻¹
(1 + i) * (m + ni)²ᵏ⁻² * (m - ni)² | (1 - i) * (m + ni)² * (m - ni)²ᵏ⁻²
...
(1 + i) * (m + ni)⁰ * (m - ni)²ᵏ | (1 - i) * (m + ni)²ᵏ * (m - ni)⁰
```
...
1. [Суммы квадратов и целые гауссовы числа, В. Сендеров, А. Спивак (Теорема 10, стр. 8)](https://kvant.mccme.ru/pdf/1999/03/kv0399senderov.pdf)
